...
<#
.SYNOPSIS
    Builds LiteRestaurant Windows installer (RestaurantPOS + PrintServer + Launcher).

.DESCRIPTION
    Run from repository root or restaurant_installer folder on a development machine with:
    - .NET 8 SDK
    - Node.js 18+
...
$ApiProject = Join-Path $RepoRoot "restaurant_back\RestaurantPOS\RestaurantPOS.csproj"
$PrintServerProject = Join-Path $RepoRoot "restaurant_back\PrintServer\PrintServer.csproj"
$LauncherProject = Join-Path $InstallerDir "LiteRestaurant.Launcher\LiteRestaurant.Launcher.csproj"
$FrontendDir = Join-Path $RepoRoot "restaurant_front"
...
Write-Host "=== LiteRestaurant installer build ==="
...
if (-not $SkipFrontendBuild) {
    # Force same-origin API so the installed SPA talks to local RestaurantPOS (:5189),
    # not whatever VUE_APP_API_URL is in restaurant_front/.env (dev LAN IP / remote).
...
Write-Host "Publishing RestaurantPOS..."
dotnet publish $ApiProject `
    -c $Configuration `
    -r $Runtime `
    --self-contained true `
    -o (Join-Path $StagingDir "RestaurantPOS") `
...
$productionSettingsTarget = Join-Path $StagingDir "RestaurantPOS\appsettings.Production.json"
Copy-Item $productionSettingsSource $productionSettingsTarget -Force

# Restaurant SPA is served from wwwroot root (not wwwroot/app)
$spaTarget = Join-Path $StagingDir "RestaurantPOS\wwwroot"
Ensure-Directory $spaTarget
Copy-Item (Join-Path $FrontendDir "dist\*") $spaTarget -Recurse -Force

# Remove launcher publish artifacts except LiteRestaurant.exe from staging root clutter
Get-ChildItem $StagingDir -File | Where-Object { $_.Name -ne "LiteRestaurant.exe" } | Remove-Item -Force

if (-not $SkipInstallerCompile) {
    Write-Host "Compiling installer with Inno Setup..."
    & $IsccPath (Join-Path $InstallerDir "LiteRestaurant.iss")
