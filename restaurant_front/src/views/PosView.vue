                  ...
                  <div class="pos-tables-toolbar-end">
                    ...
                    >
                      <div class="pos-table-action-row pos-table-action-row--ops">
                        <div
                          v-if="selectedTableId && selectedTable && selectedTable.status === 'Occupied'"
                    ...
                    <div class="pos-cart-checkout-segment pos-cart-checkout-segment--summary">
                      ...
                      <div class="pos-cart-checkout-btn-row pos-cart-checkout-summary-actions">
                        <template v-if="isCreditPayment">
                          <button
                            type="button"
                            class="pos-action-btn pos-action-btn-primary pos-cart-checkout-action-btn pos-cart-checkout-action-btn--credit"
                            @click="openCreditCheckoutModal"
                            :disabled="totalCardItems <= 0 || orderPersisting || !hasCreditAccountSelected"
                            :title="$t('confirmCreditAndPrint') || 'تأكيد وطباعة'"
                          >
                            <b-icon icon="wallet2" class="me-1"></b-icon>
                            {{ $t("confirmCreditAndPrint") || "تأكيد وطباعة" }}
                          </button>
                        </template>
                        <template v-else>
                          <button
...
export default {
  ...
  computed: {
    ...
    showPosCheckoutBar() {
      if (this.posFloorPlanGateVisible) return false;
  ...
  methods: {
    ...
    async onPosFloorPlanTableClick(table, event) {
      await posTableSelectMixin.methods.onPosFloorPlanTableClick.call(this, table, event);
      this.$nextTick(() => {
        if (this.$refs.posQuickSearchInput) {
          this.$refs.posQuickSearchInput.focus();
        }
  ...
  `;
    ...
    openOrderNotesModal(mode = "pay") {
      ...
      if (mode === "credit-print") {
        ...
        this.checkoutSubmitMode = "credit-print";
      } else {
    ...
    openCreditCheckoutModal() {
      if (!this.hasCreditAccountSelected) {
        this.openCreditPaymentModal();
        return;
      }
      this.openOrderNotesModal("credit-print");
    },
    ...
    async confirmAddOrder() {
      ...
      if (this.orderForSend.paymentMethod === "Credit") {
        const toastPosition = this.getOrderPersistToastPosition
          ? this.getOrderPersistToastPosition()
          : "top-right";
        if (!this.validateCreditForOrder(toastPosition)) {
...
.pos-cart-checkout-bar .pos-cart-checkout-action-btn--credit {
  background: linear-gradient(135deg, #fbbf24 0%, #d97706 42%, #b45309 100%) !important;
  box-shadow:
    0 2px 10px rgba(217, 119, 6, 0.38),
    inset 0 1px 0 rgba(255, 255, 255, 0.18) !important;
}

.pos-cart-checkout-bar .pos-cart-checkout-action-btn--credit:hover:not(:disabled) {
