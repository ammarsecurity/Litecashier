...
export default {
  ...
  methods: {
    ...
    getDailySequenceFromTx(tx) {
      const order = this.getOrderFromTx(tx);
      const seq = order?.dailySequenceNumber ?? order?.DailySequenceNumber ?? tx?.dailySequenceNumber;
      const n = Number(seq);
      return Number.isFinite(n) && n > 0 ? n : null;
    },
    formatOrderNumber(item) {
      if (!item) return null;
      const seq = item.dailySequenceNumber != null ? Number(item.dailySequenceNumber) : null;
      const code = item.orderCode || null;
      if (seq && code) return `#${seq} — ${code}`;
      if (seq) return `#${seq}`;
      if (code) return code;
    ...
    formatOrderNumberFromTx(tx) {
      ...
      return this.formatOrderNumber({
        dailySequenceNumber: this.getDailySequenceFromTx(tx),
